...
func TestValidatePathTemplates(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "encoded dot segment", templates: []string{"/users/%2e/orders"}, wantErr: true},
		{name: "encoded dot dot segment", templates: []string{"/users/%2E%2E/orders"}, wantErr: true},
		{name: "mixed encoded dot dot segment", templates: []string{"/users/.%2e/orders"}, wantErr: true},
