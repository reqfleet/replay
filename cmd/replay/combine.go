...
func runCombine(args []string, stdout, stderr io.Writer) int {
	...
	summary, err := combineFiles(*logPath, *outPath, format)
	if err != nil {
...
func combineFiles(inputPath, outputPath, format string) (summary recorder.CombineSummary, returnErr error) {
	...
	input, err := parser.OpenFile(inputPath, format)
	if err != nil {
	...
	defer func() {
		...
		if returnErr == nil {
			...
			returnErr = fmt.Errorf("%v; %w", returnErr, cleanupError)
		}
	}()
