...
captured response expectations. Raw Envoy observations are preprocessing input,
not engine input.
...
timestamp, stream ID, response duration, or sequence. HTTP/1.1 records omit
`stream_id`; replay uses internal stream ID `1` for those requests.
