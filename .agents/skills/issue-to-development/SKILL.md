...
7. Begin development: open the relevant files, implement changes, and run project-specific checks (linters, static type checks, unit tests). Examples:
	 - Go: Do not hardcode test or build commands here — consult the replay `Makefile` for repository-specific Go checks and `make` targets (for example: `make test`, `make e2e`, `make alltests`, or `make build`). Follow the Makefile-defined checks before running tests.
