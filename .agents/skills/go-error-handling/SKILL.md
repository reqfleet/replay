...
From the host repository, run `make devbox-ssh` before verification. Run every project `make` target and `go test` command only in the resulting VM shell at `/workspace/replay`, never directly on the macOS host. `make devbox`, `make devbox-ssh`, and `make devbox-stop` are host-only VM lifecycle exceptions.
