...
func TestOutcomeAggregation_DryRunSerialized(t *testing.T) {
	...
	summary := e.replayConnectionSerialized(context.Background(), client, requests, nil)
...
func TestOutcomeAggregation_SendError(t *testing.T) {
	...
	client, transport := e.makePerConnectionClient(false)
	defer transport.CloseIdleConnections()
	summary := e.replayConnectionSerialized(context.Background(), client, []model.Event{bad}, nil)
