...
- `make devbox`, `make devbox-ssh`, `make devbox-stop`, and `make devbox-recreate` are host-only VM lifecycle exceptions.
- `DEVBOX_PROJECT_DIR` controls the host directory mounted at `/workspace/replay` and defaults to the current checkout. Override it only when the VM should mount a different checkout, then run `make devbox-recreate` to apply the change to an existing VM.
